...
| Table | Contenu |
|---|---|
| `commissariats`, `mairies` | `id`, `name` (150, unique), `code` (30, unique, nullable), `is_active`, timestamps, `deleted_at`. Adresse, téléphone, région, commune : À VALIDER AVEC LE CLIENT. `mairies` a aussi `card_template` (string, défaut `officiel`, `App\Enums\VgtCardTemplate`, W13) `logo_path` et `monument_path` (nullables, logo et image du monument imprimés sur la carte VGT) |
...
| `sms_messages` | SMS de la plateforme (W14) : `event`, `phone`, `message`, `status` (`queued`, `sent`, `failed`), `driver`, `provider_reference`, `error`, sujet (`subject_type`, `subject_id`), `sent_at`, soft delete. Numéro et texte exclus de l'audit |
...
~~Workflow mairie (statuts, rejet)~~ → PROPOSITION TECHNIQUE d'Amadou pour W11 (voir `App\Enums\DemandeVgtStatus`) : en_attente → validée ou rejetée (motif obligatoire, le commissaire corrige et resoumet) → payée (W12) → retirée (W13, confirmation de remise de la carte physique par la mairie).

~~Confirmation de remise de la carte à la mairie (W13) ; nature et effet de la « taxe à payer » du contrôle de police~~ → tranché avec le développeur (2026-09-29), à partir du mockup du cahier (§9 Retrait VGT, lignes « Taxe à payer pour non en jour » et « Taxe à payer pour non dans la base ») : la « taxe à payer pour non en jour » est la **même majoration** déjà calculée et payée en W11/W12 (`surcharge_amount`) — l'écran de retrait ne fait qu'afficher/imprimer, aucune nouvelle règle ni second paiement. Le cas « non enregistré / non dans la base » est **hors périmètre** : une demande VGT (W11) exige déjà une moto existante (créée au commissariat, W8), ce cas ne peut donc pas survenir par le flux normal de VigiMoto ; le mobile police (M1) n'a donc plus de blocage client non plus, l'endpoint de contrôle (§8) suffit déjà.
...
**Design de la carte VGT imprimée (W13)** — le cahier ne décrit aucun visuel, seulement les champs (§9 Retrait VGT). Demande explicite du développeur (2026-09-29) : plusieurs modèles sélectionnables par mairie, dont un « officiel » aux couleurs du Mali. **À VALIDER AVEC LE CLIENT / la DGI avant mise en production** : le modèle `officiel` (`resources/views/demandes-vgt/_card_templates/officiel.blade.php`) affiche les couleurs nationales et un sceau — un dessin original (étoile et cercles), pas une reproduction des armoiries réelles de l'État, qu'aucune source publique ne documente pour la vignette elle-même (recherche du 2026-09-29 : le DGI n'a publié aucune photo du document physique, et met en garde contre les reproductions frauduleuses de vignettes — risque de contrefaçon d'un symbole d'État à faire trancher par le client avant tout hébergement public). Les 4 autres modèles (classique, moderne, premium, minimaliste) sont des habillages VigiMoto sans symbole national.
...
- **Sidebar** (`partials/sidebar.blade.php`, réorganisée le 2026-09-30) : recherche dans le menu, Tableau de bord, entrées des manifestes (filtrées par permission, si la route existe) : le **registre** (propriétaires, motos, déclarations, motos retrouvées) tient en **une seule entrée « Registre motos »**, les écrans voisins se rejoignent par une **barre d'onglets** (`partials/registre-tabs`, un onglet par écran autorisé) ; les autres entrées sont rangées par domaine (Vignettes VGT, Administration — titre de groupe seulement s'il y a au moins 2 entrées). Réglages dans `config/sidebar.php` (aucun manifeste modifié ; une entrée inconnue tombe dans « Général »). Les modules « à venir » déjà livrés dans un autre écran (retrait et paiements, dans Demandes VGT) ne sont plus listés (`embedded_planned`). Une **pastille orange** indique le nombre d'actions en attente pour l'utilisateur (`RealDashboardData::pendingDemandesCount()`, via `ModuleServiceProvider`). Les « Modules à venir » (fiches sans donnée) sont **repliés** dans un groupe (ouvert automatiquement sur leur page), puis **Paramètres** tout en bas, et une carte utilisateur collée en pied de menu (Rôles, Permissions, Attribution, **Audit** et **Système** s'ouvrent depuis son sous-menu, `configuration/_menu.blade.php` : un écran de Paramètres n'a pas d'entrée de sidebar, son manifeste déclare `navigation => []`).
...
2. **Tableaux** : `<table class="table" id="…">` avec un `<thead>` **sans classe de couleur** : le style du thème (libellés en majuscules, petit corps, gris) vient de `numerise.css`. Pas de `table-dark`, `table-primary` ni de fond ad hoc. Le tableau est initialisé en DataTables (libellés français), sauf les fiches clé / valeur (`table table-sm` sans DataTables). Boutons d'action `btn btn-primary btn-sm` avec une icône Boxicons, dans une colonne étroite (`width="10%"`). Le thème impose `white-space: nowrap` dans `.table-responsive` et `scrollX: true` aux DataTables : une cellule de texte libre reçoit la classe `cell-wrap` (avec `text-break`), et un tableau à colonnes larges passe `scrollX: false`, sinon les boutons d'action sortent de la carte.
...
- **Finition visuelle** (2026-09-30, demande du développeur ; touche au thème du socle, **à relire par Moustapha**) : bloc additif en fin de `numerise.css` (fond, cartes à ombre douce, bandeaux en dégradé, cartes chiffrées du tableau de bord, sidebar à pastille active, tableaux, boutons, modales, pastilles de statut `bg-*-subtle` absentes de Bootstrap 5.0). Le gabarit impératif ci-dessus est inchangé. **Boutons** : système unifié en fin de `numerise.css` — primaire, succès et danger en dégradé avec reflet au survol, secondaire blanc à contour, contour qui se remplit, focus visible ; les boutons d'action à icône seule des tableaux (`btn btn-primary btn-sm` avec une seule icône Boxicons) deviennent des pastilles teintées **selon l'icône** (`bx-edit` bleu, `bx-trash` rouge, `bx-check-shield` vert, `bx-money` ambre, `bx-id-card` indigo) : garder ces icônes pour ces actions.
- **Tableau de bord réel** (2026-09-30, `RealDashboardData`, `resources/views/dashboard/_body.blade.php`) : accueil personnalisé (salutation, rôle, institution, date), **« À traiter maintenant »** (tâches par permission : demandes à valider, paiements à confirmer, cartes à remettre, demandes rejetées à corriger, chacune avec son effectif et un lien qui ouvre la liste déjà filtrée par `?statut=`), chiffres des modules construits (propriétaires, motos dont volées, déclarations, motos retrouvées, demandes VGT, encaissé du mois), **parcours des demandes VGT** (barre proportionnelle par statut, cliquable) et dernières demandes. Tout est cloisonné comme les écrans (`DemandeVgt::visibleTo`, scopes d'institution). Avec `DASHBOARD_MOCK=true` (D25) les cartes et tableaux restent fictifs (seul l'accueil personnalisé est réel) : mettre `false` en local pour voir les vrais chiffres.
- **Écrans de liste enrichis** (2026-10-01, modèle : Propriétaires, Motos, Déclarations, Motos retrouvées, Informations, Utilisateurs ; Commissariats et Mairies n'ont que les pastilles) : cellules regroupées (identité en gras, détail en `small text-muted`), **pastilles de filtre** `nv-chips` avec effectif (jetons cachés `d-none` dans une colonne, filtrés par `DataTable().column(n).search(token)` ; jetons sans recouvrement, ex. `type-vol` et non `vol`, sinon « moto-volee » correspond) et **fiche de consultation en modale** (`<module>/_fiche.blade.php`, panneaux `nv-panel` / `nv-dl`, bouton `bx-show`). Aucune donnée secrète dans une fiche. À reproduire pour tout nouvel écran de liste.
...
- **Select2 en recherche (AJAX)** — ajout socle (`partials/foot.blade.php`), **à relire par Moustapha** : un champ dont la liste complète serait trop lourde à précharger (propriétaire, moto…) reçoit la classe `ajax-select` (au lieu de `single-select`) plus `data-search-url="{{ route(...) }}"` et `data-placeholder="…"` ; seule l'option déjà choisie (édition, ou re-soumission après erreur) est pré-rendue côté serveur, le reste se charge en tapant (délai 250 ms, 20 résultats). Le contrôleur expose un `search(Request $request): JsonResponse` sur le même modèle que `index()` (même permission, même cloisonnement), répondant `{"results": [{"id", "text"}, ...]}` (format attendu par Select2). Exemples : `ProprietaireController::search()` (utilisé par W8), `MotoController::search()` (utilisé par W9). Un champ à liste courte et fixe (genre, type d'acte, rôle…) reste en `single-select` classique : ce n'est pas « beaucoup de données ».
...
npx expo start            # scanner le QR code avec Expo Go (réponse « UnexpectedServerError » : `npx expo start --offline`)
```
...
| W12 | Module **Paiements** | 3 | W11 | non | Amadou |
| W13 | Module **Retrait VGT** | 3 | W11, W12 | non | Amadou |
...
- **W12 Paiements** : reste sur l'écran Demandes VGT (le cahier ne montre pas d'écran Paiements séparé, §9) — la mairie confirme le paiement (« Confirmer le paiement », date), une demande `validee` passe à `payee` (`DemandeVgtController::confirmPaiement()`, permission `demandes-vgt.confirm_payment`, PROPOSITION TECHNIQUE, §5). Le montant est celui déjà fixé à la demande (W11) ; le code marchand reste une référence saisie par le commissaire, sans passerelle de paiement réelle. SMS de confirmation : W14, pas encore câblé. **Fait** (`tests/Feature/DemandesVgt/DemandeVgtScreenTest.php`). Pas d'API dédiée pour M4 : à construire avec les routes publiques D32.
- **W13 Retrait VGT** : reste aussi sur l'écran Demandes VGT (même raison qu'au-dessus, W12) — la mairie confirme le retrait de la carte physique (« Retrait de la carte VGT », date), une demande `payee` passe à `retiree` (`DemandeVgtController::confirmRetrait()`, permission `demandes-vgt.confirm_retrait`). Aperçu (fiche propriétaire) et impression (carte imprimable auto-générée en JS, iframe caché + `print()`, sans page ni route dédiée) accessibles tant que la demande est `payee` ou `retiree`, pour la mairie comme pour le commissariat qui l'a déposée. Cas « taxe à payer » et « non dans la base » : PROPOSITION TECHNIQUE tranchée par Amadou (§5, 2026-09-29). **Carte imprimée réelle** (2026-09-30) : carte plastique CR80 (85,6 × 54 mm), `@page` exact sans marge. Les 5 modèles reproduisent les vraies vignettes motos du District de Bamako (photos 2017, 2018, 2023, 2025 et 2026 fournies par le développeur) : `officiel` (2026, blanc/gris, pastille jaune, QR Code, monument), `jaune` (2025, QR Code, monument rose, sceaux), `rose` (2023, guillochis bleus, titre doré, sceau bleu), `classique` (2018, VIGNETTE rouge, hologramme, signature), `ancien` (2017, lignes ondulées, VIGNETTE espacé bordeaux). Recto : année verticale, numéro, armoiries de la commune, « RÉPUBLIQUE DU MALI / Un Peuple - Un But - Une Foi », mairie émettrice, VIGNETTE, montant, pictogramme moto, cadre du numéro, bandeau « ENGINS A 2 ROUES », mention contre la contrefaçon. **Verso : uniquement le nom et le numéro du détenteur**, comme sur les vraies cartes. Différences assumées : le cadre porte le **matricule** (pas de numéro d'engin, §5) ; le bandeau indique le genre saisi (`type_or_brand`), VigiMoto ne connaît pas la cylindrée ; armoiries, monument, sceaux et signature sont des **dessins originaux**, pas les emblèmes officiels (à valider client/DGI). Le QR Code (modèles 2025 et 2026) encode l'adresse de vérification signée de la vignette (voir ci-dessous) ; il est dessiné côté navigateur par `public/assets/plugins/qrcode/qrcode.js` (qrcode-generator, MIT, licence dans `public/assets/licenses/qrcode-generator-LICENSE.txt`, aucun CDN). Migration `2026_09_30_100001` : `moderne`→`rose`, `premium`→`jaune`, `minimaliste`→`ancien`. **Images réelles des cartes** (2026-09-30) : les pictogrammes viennent des photos de vraies vignettes fournies par le développeur, détourés avec GD (script hors dépôt, seuils par distance à la couleur de fond) dans `public/assets/images/vignette/` : armoiries `arms-bamako.png` (2017, défaut), `arms-2023.png`, `arms-2018.png`, silhouette de moto `scooter-flat.png` (masque coloré selon le modèle) et `scooter-photo.png` (2018), hologramme `holo-gris.png`, sceau `seal-bleu.png` (2023). Les titres utilisent la police **Archivo Black** (SIL OFL, `public/assets/fonts/`, licence `public/assets/licenses/archivo-black-OFL.txt`) au lieu d'Arial Black, absente de beaucoup de postes. **À valider avec le client / la DGI** : ces armoiries sont celles du District de Bamako, reproduites d'une vraie vignette : une autre mairie doit téléverser son propre logo (réglage ci-dessous) ; la signature du maire reste un gribouillis générique, jamais la signature réelle. **Logo de la carte paramétrable** : chaque mairie peut téléverser son logo (PNG, JPEG ou WebP, 100 × 100 px minimum, 1 Mo maximum, SVG refusé) depuis « Modèles de carte » sur l'écran Demandes VGT (`MairieCardTemplateController::updateLogo()/resetLogo()`, permission `mairies.update`, fichier sur le disque `public` sous `mairies/logos/`, ancien fichier supprimé au remplacement) ; il remplace, sur les 5 modèles, l'image par défaut (armoiries dessinées, `_card_parts/arms`), et « Revenir à l'image par défaut » le retire. Le monument des modèles 2025 et 2026 est un dessin par défaut du Monument de l'Indépendance, **remplaçable par une photo réelle** (même écran, `updateMonument()/resetMonument()`, PNG/JPEG/WebP, 100 × 100 px minimum, 2 Mo maximum, cadrée en portrait ; un PNG détouré rend mieux qu'une photo avec ciel). La photo elle-même n'est pas dans le dépôt : à téléverser depuis l'interface (droits d'usage à vérifier). **Vérification par QR Code** (2026-09-30, PROPOSITION TECHNIQUE — À VALIDER AVEC LE CLIENT, le QR Code est un module optionnel du cahier) : le QR des cartes ouvre `GET /verifier/{référence}?s={signature}` (`VignetteVerificationController`, `App\Services\Vgt\VignetteVerifier`), page **publique** sans compte, limitée par IP (`throttle:public`, route dans la liste blanche du test d'architecture), qui répond « Vignette valide » (payée ou retirée, année en cours), « expirée » (année passée), « non délivrée » (paiement pas encore confirmé) ou « non reconnue ». La signature est un HMAC de la référence avec la clé de l'application : on ne peut pas deviner une vignette en essayant des numéros, et une référence ou une signature fausse renvoie « non reconnue ». Aucune donnée personnelle (ni nom, ni téléphone) : matricule, genre, année, mairie, référence. **Choix du modèle à l'impression** (`resources/views/demandes-vgt/_retrait.blade.php`) : la modale de retrait est une synthèse (frise des étapes demande → validation → paiement → remise, moto et propriétaire, montant détaillé avec majoration) ; « Imprimer la carte » la ferme et ouvre une **modale distincte** de choix du modèle (liste des 5 modèles en miniature, grand aperçu recto + verso, modèle de la mairie présélectionné, bouton Retour), puis « Imprimer ce modèle » charge la carte recto + verso dans un iframe caché et lance l'impression. Le tableau regroupe l'information (moto + genre, propriétaire + téléphone, total + arriéré) et des pastilles filtrent par statut avec leur effectif. Chaque carte est servie par `GET /demandes-vgt/{id}/carte/{modèle}` (`DemandeVgtController::card()`, permission `demandes-vgt.view`, cloisonné par `visibleTo`, demandes payées ou retirées seulement). Uniquement des données réelles de la demande. Les 5 modèles reprennent les couleurs des vignettes réelles : `officiel` (blanc/gris, 2026), `classique` (blanc/rouge, 2018), `moderne` (rose, 2023), `premium` (jaune, 2025), `minimaliste` (noir et blanc, sans guillochis). **Modèles de carte** (demande explicite du développeur, 2026-09-29) : 5 habillages visuels au choix par mairie (`App\Enums\VgtCardTemplate` : officiel, classique, moderne, premium, minimaliste ; partiels `resources/views/demandes-vgt/_card_templates/*.blade.php`), réglés depuis un bouton « Modèles de carte » sur l'écran Demandes VGT (`mairies.card_template`, gardé par la permission `mairies.update` déjà du manifeste ; l'écran Mairies (W2) gère le CRUD, ces réglages de carte (modèle, logo, monument) sont accessibles depuis **deux écrans** : « Modèles de carte » sur Demandes VGT et le bouton carte d'identité de chaque ligne de l'écran Mairies, qui utilisent le même bloc `demandes-vgt/_card_settings_block.blade.php` et reviennent à l'écran d'origine (champ `back`, seule la valeur `mairies` est reconnue) ; ils partagent `routes/web/mairies.php` avec W2). **Fait** (`tests/Feature/DemandesVgt/DemandeVgtScreenTest.php`, `tests/Feature/DemandesVgt/CardTemplateTest.php`).
